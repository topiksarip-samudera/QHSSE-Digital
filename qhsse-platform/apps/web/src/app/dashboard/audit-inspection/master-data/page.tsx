...
export default function AuditMasterDataPage() {
  const [data, setData] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [seeded, setSeeded] = useState<any>(null);

  useEffect(() => { (async () => { try { const r = await auditInspectionApi.getMasterData(); setData(r.data||[]); } catch(e){console.error(e);} finally{setLoading(false);} })(); }, []);
  const handleSeed = async () => { try { const r = await auditInspectionApi.seedDefaults(); setSeeded(r.data); setTimeout(()=>window.location.reload(),1500); } catch(e){console.error(e);} };

  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between"><h1 className="text-2xl font-bold text-gray-900">Audit Master Data</h1><button onClick={handleSeed} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">Seed Default Data</button></div>
