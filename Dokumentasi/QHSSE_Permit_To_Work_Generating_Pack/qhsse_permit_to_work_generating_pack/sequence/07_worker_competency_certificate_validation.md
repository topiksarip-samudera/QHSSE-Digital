...
Membangun validasi training, competency, certificate, medical check optional, dan block/warn policy.
...
```text
Worker training check
Certificate expiry check
Competency matrix check
Medical check optional
Contractor worker validation
Block/warning validation
Validation result per worker
```
...
```text
SELESAI PTW SEQUENCE 07: Worker Competency & Certificate Validation
```
